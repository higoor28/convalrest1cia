...
generateUnificadorButton.addEventListener("click", async () => {
  ...
  try {
    ...
    setUnificadorStatus(
      `Controle unificado com ${result.recordCount} registro(s) e ${result.signatureCount} assinatura(s).`,
    );
    downloadTextFile(
...
generateDespachoUnificadorButton.addEventListener("click", async () => {
  ...
  try {
    ...
    setDespachoUnificadorStatus(
      `Despacho unificado com ${result.filledCount} item(ns) preenchido(s)${automaticText} e ${result.signatureCount} assinatura(s).`,
    );
    downloadTextFile(
...
function extractRoleAfterHyphen(value) {
  const role = valueText(value);
  const separatorIndex = role.search(/[-–—]/);
  return separatorIndex >= 0 ? role.slice(separatorIndex + 1).trim() : role;
}
...
function buildMemorandoHtml(template, cells) {
  ...
  const senderRole = extractRoleAfterHyphen(signerRole);
  ...
  if (!senderRole) {
    throw new Error("A celula F7, usada como funcao do remetente do memorando, esta vazia.");
  }
  ...
  html = replaceAll(html, "{{MEMORANDO_REMETENTE_FUNCAO}}", escapeHtml(senderRole));
